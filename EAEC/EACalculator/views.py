...
def index(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            (profile, mp) = selector.profileSelector(age, spare_money, free_time, fitness)
            datepicker = dataPicker(profile, mp, near_water)
            table = datepicker.getTable(profile, mp, near_water)
